...
Arthub Ensor is a fork of the [Arthub Flanders](https://arthub.vlaamsekunstcollectie.be) platform ([Github project](https://github.com/vlaamsekunstcollectie/Arthub-Frontend/tree/iiif-testbed)), a customised [Project Blacklight](https://github.com/projectblacklight/blacklight) installation written in Ruby on Rails. It utilizes a customized [Imagehub](https://github.com/Hero-Solutions/ImageHub/tree/ensor) installation to generate [IIIF Presentation API](https://iiif.io/api/presentation/2.1/) in order to display high-resolution images.

The Imagehub provides a link between [ResourceSpace](https://www.resourcespace.com) and the [Datahub](https://github.com/thedatahub/Datahub). High-resolution images are uploaded in Resourcespace and data from the Datahub is added to these resources through the use of Imagehub commands. IIIF Presentation API manifests are then generated for each resource and a link to each manifest is added to the appropriate record inside the Datahub. Through the use of Catmandu fixes, each Datahub record can be transformed into an Apache Solr document to be displayed on the Arthub with metadata about the work along with the appropriate high-resolution image inside a IIIF viewer.
...
Once the Imagehub is correctly installed, it is important to edit the appropriate config values in the RS_ptif ResourceSpace plugin so it correctly points to your newly installed Imagehub. Since this installation does not make use of authentication or a remote standalone image viewer, certain values can be left empty. The following lines may in this instance be appended at the end of the ResourceSpace configuration file:
```
# RS_ptif configuration
